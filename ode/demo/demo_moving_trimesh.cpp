...
int VertexCount2 = 0, IndexCount2 = 0;
...
void stlLoad(string fileName, int &VertexCountF, int &IndexCountF, int* IndicesF, dVector3* VerticesF, int reading_method) {
	cout << "start loading\n";
	...
	if ( file.is_open() ) {
		...
		while ( word == "facet" ) {
			...
			iss3 >> word >> VerticesF[i][0] >> VerticesF[i][1] >> VerticesF[i][2];
			if ( reading_method == 1 ) {
				getline(file, line);
				istringstream iss4(line);
				iss4 >> word >> VerticesF[i + 2][0] >> VerticesF[i + 2][1] >> VerticesF[i + 2][2];
				getline(file, line);
				istringstream iss5(line);
				iss5 >> word >> VerticesF[i + 1][0] >> VerticesF[i + 1][1] >> VerticesF[i + 1][2];
			}
			else {
				getline(file, line);
				istringstream iss4(line);
				iss4 >> word >> VerticesF[i + 1][0] >> VerticesF[i + 1][1] >> VerticesF[i + 1][2];
				getline(file, line);
				istringstream iss5(line);
				iss5 >> word >> VerticesF[i + 2][0] >> VerticesF[i + 2][1] >> VerticesF[i + 2][2];
			}
			getline(file, line);
			getline(file, line);
			getline(file, line);
			...
			for ( int j = 0; j < 3; j++ ) {
				IndicesF[IndexCountF] = IndexCountF; IndexCountF++;
				//cout << "vertex " << i + j << ": " << Vertices[i + j][0] << "\t" << Vertices[i + j][1] << "\t" << Vertices[i + j][2] << "\n";
			}
			i += 3;
			if ( IndexCountF >= MAX_INDEX - 9 ) {
				cout << "vertex reach max\n";
				break;
			}
		}
		VertexCountF = i;
		cout << "vertex: " << VertexCountF << "\n index: " << IndexCountF << "\n";
	}
	file.close();
	// 	cout << "build data\n";
...
static void command(int cmd) {
	...
	cmd = locase(cmd);
	if ( cmd == 'b' || cmd == 's' || cmd == 'c' || cmd == 'x' || cmd == 'm' || cmd == 'y' || cmd == 'n' ) {
		if ( num < NUM ) {
			i = num;
			num++;
		}
		...
		if ( cmd == 'b' ) {
			...
			dTriMeshDataID new_model = dGeomTriMeshDataCreate();

			stlLoad(cubefile, VertexCount2, IndexCount2, &Indices2[0], &Vertices2[0], 1);
			dGeomTriMeshDataBuildSingle(new_model, &Vertices2[0], 3 * sizeof(float), VertexCount2,
										(dTriIndex*)&Indices2[0], IndexCount2, 3 * sizeof(dTriIndex));
			obj[i].geom[0] = dCreateTriMesh(space, new_model, 0, 0, 0);
			dGeomSetData(obj[i].geom[0], new_model);
			dMassSetTrimesh(&m, DENSITY, obj[i].geom[0]);
			printf("mass at %f %f %f\n", m.c[0], m.c[1], m.c[2]);
			dGeomSetPosition(obj[i].geom[0], -m.c[0], -m.c[1], -m.c[2]);
			dMassTranslate(&m, -m.c[0], -m.c[1], -m.c[2]);
...
static void simLoop(int pause) {
  ...
  {for ( int i = 0; i < IndexCount2 / 3; i++ ) {
	  dReal v[9] = { // explicit conversion from float to dReal
		  Vertices2[i * 3 + 0][0],
		  Vertices2[i * 3 + 0][1],
		  Vertices2[i * 3 + 0][2],
		  Vertices2[i * 3 + 1][0],
